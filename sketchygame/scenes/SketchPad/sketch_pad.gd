...
var action: SketchAction = SketchAction.Draw;
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if mouse_event.button_mask == MOUSE_BUTTON_MASK_LEFT:
		...
		if action == SketchAction.Draw:
			mesh_tool.create_mesh(trail, temp_drawing);
		elif action == SketchAction.Erase:
			_parse_erase(trail[-1])
	else:
		dragging = false;
...
func _parse_erase(point: Vector2) -> void:
	var mesh_to_delete := mesh_tool.delete_mesh(point, visible_layers.get_children());

	if mesh_to_delete == null:
		return;

	mesh_to_delete.reparent(unvisible_layers);
	mesh_to_delete.visible = false;
